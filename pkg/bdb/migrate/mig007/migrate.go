...
func updateEncodingManifest() func(*zerolog.Logger, *bolt.DB, *bolt.DB) error {
	return func(log *zerolog.Logger, roDB *bolt.DB, rwDB *bolt.DB) error {
		...
		if err := roDB.View(func(rtx *bolt.Tx) error {
			...
			b, err := mig.SetBucket(rtx, bdb.ManifestPath)
			if err != nil {
				return err
			}
			...
			// re-encode body value.
			{
				...
				if err := mig.SetKey(wtx, bdb.ManifestPath, []byte(bdb.BodyKey), bodyBuf); err != nil {
					return err
				}
			}
			...
			// re-encode metadata value.
			{
				...
				if err := mig.SetKey(wtx, bdb.ManifestPath, []byte(bdb.MetadataKey), metadataBuf); err != nil {
					return err
				}
			}
			...
			// copy model value as-is.
			{
				...
				if err := mig.SetKey(wtx, bdb.ManifestPath, []byte(bdb.ModelKey), modelValue); err != nil {
					return err
				}
			}
			return wtx.Commit()
