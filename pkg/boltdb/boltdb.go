...
// bucket level operations
	SetBucket(ctx context.Context, tx *bolt.Tx, path []string) error
	DeleteBucket(ctx context.Context, tx *bolt.Tx, path []string) error
	ListBuckets(ctx context.Context, tx *bolt.Tx, path []string, pagination) (buckets []string, pagination, err error)
	BucketExists(ctx context.Context, tx *bolt.Tx, path []string) (bool, error)

	// key level operations
	ListKeys(path []string, pageToken string, pageSize int32, opts []Opts) (keys []string, nextToken string, totalSize int32, err error)
	KeyExists(path []string, key string, opts []Opts) bool

	// value level operations
	Read(path []string, key string, opts []Opts) ([]byte, error)
	Write(path []string, key string, value []byte, opts []Opts) error
	Delete(path []string, key string, opts []Opts) error
	List(path []string, pageToken string, pageSize int32, opts []Opts) (keys []string, values [][]byte, nextToken string, totalSize int32, err error)
